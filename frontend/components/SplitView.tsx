  ...
  return (
    <div className="rounded-2xl border-2 border-sky-200 bg-sky-50 p-4 stagger-in">
      <div className="flex flex-wrap items-center gap-3">
        <div className="text-sm font-extrabold text-sky-900 inline-flex items-center gap-2"><Icon name="firewall" className="w-4 h-4" /> {s.splitTitle}</div>
        <button onClick={run} disabled={loading || !query.trim()} className="pressable ml-auto text-sm font-bold px-4 py-3 rounded-full bg-sky-600 text-white hover:bg-sky-700 disabled:opacity-50 touch-48">
          {loading ? s.splitLoading : s.splitRun}
        </button>
      </div>
      <p className="text-xs text-sky-800 mt-1 leading-relaxed">{s.splitHint}</p>
      {(india || intl) && (
        <div className="mt-4 grid grid-cols-1 lg:grid-cols-2 gap-4">
          {[
            { label: s.india, icon: "india" as IconName, color: "border-saffron", data: india },
            { label: s.world, icon: "world" as IconName, color: "border-indiaBlue", data: intl },
          ].map((col) => (
            <div key={col.label} className={`rounded-2xl border-2 bg-white overflow-hidden stagger-in ${col.color}`} style={{ animationDelay: "80ms" } as React.CSSProperties}>
              <div className="px-3 py-2 text-xs font-extrabold tracking-widest flex items-center gap-2 bg-stone-50 border-b border-stone-200">
                <Icon name={col.icon} className="w-3.5 h-3.5" />{col.label} {col.data && <span className="ml-auto"><ConfidenceBadge score={col.data.confidence.score} abstain={col.data.confidence.abstain} label={s.confidence} /></span>}
              </div>
              {col.data ? (
                <div className="p-4">
                  ...
                  <div className="mt-3 text-xs"><CitationPane citations={col.data.citations.slice(0,2)} corpusVersion={col.data.corpus_version} lang={lang} /></div>
                </div>
              ) : (
                <div className="p-8 text-center text-xs text-stone-400">{s.splitNoData}</div>
              )}
            </div>
          ))}
        </div>
