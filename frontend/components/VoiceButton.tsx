...
export function VoiceButton({ onTranscript, lang = "hi" }: { onTranscript: (v: string) => void; lang?: string }) {
  ...
  async function toggle() {
    ...
    if (!SpeechRecognition) {
      // Used to fire `alert(s.errorHint)`, which told the reader to check
      // whether the backend was running — the wrong diagnosis, delivered as a
      // modal. Now it is an inline note under the button, and the text is
      // actually about speech.
      setUnsupported(true);
      return;
    }
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      ...
      {unsupported && (
        <p role="status" className="max-w-xs text-center text-xs leading-relaxed text-amber-900 bg-amber-50 border border-amber-200 rounded-xl px-3 py-2">
          {s.voiceUnsupported}
        </p>
      )}
    </div>
